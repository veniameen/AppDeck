...
| `tests/proxy_bridge_test.sh build/AppDeck.app` | Part of `make verify`, right after the static bundle check. The proxy bridge (`Contents/MacOS/appdeck-proxy`) against offline loopback fixtures (`tests/proxy_fixture.py`): plain and CONNECT requests through an HTTP upstream with Basic auth and a SOCKS5 upstream with a login (names, IPv4, IPv6), failover past a dead port and a wrong login, 502 for a rejected login, every `check` result, configuration errors (exit 2, no credential echoed), 8 MiB byte-exact transfers both ways, 50 parallel requests, exit with the watched process or on stdin EOF before `watch`, and a 127.0.0.1-only listener. `APPDECK_PROXY_BIN` runs it against another build of the helper (e.g. with sanitizers). |
