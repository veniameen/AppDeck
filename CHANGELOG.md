...
### Added
- Proxy support. A **Proxy** page lists HTTP (CONNECT) and SOCKS5 proxies with one or more addresses in the provider's `host:port[:login:password]` form, a common login, a **Check** that opens a tunnel to `api.openai.com:443` through every address, and the default proxy for all profiles; a card's **•••** menu picks a profile's own (default, none or a specific proxy) and the status line shows it. A profile behind a proxy starts with a private loopback bridge (`appdeck-proxy`) that adds the credentials, fails over along the address list and lives as long as the window; the app gets `--proxy-server`, `--disable-quic`, a WebRTC policy that keeps UDP inside the proxy and the `*_PROXY` environment. Limit checks use the same proxy. Passwords are kept in `proxies.plist` (0600), never in `state.plist` or the diagnostics.
